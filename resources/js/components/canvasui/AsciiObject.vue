...
export interface AsciiObjectInstance {
  ...
  /** Ease the camera back to the angle it started at. */
  resetView: () => void;
...
export function createAsciiObject(
  ...
  // Grabbing the model again cancels an in-flight return
  ...
  function tick(time: number) {
    ...
    if (returningHome) {
      const k = reducedMotion ? 1 : 1 - Math.exp(-delta / 0.22);
      camera.position.lerp(homePosition, k);
      if (camera.position.distanceTo(homePosition) < 0.002) {
...
// Vue lists every declared prop, so unset ones would spread `undefined` over the defaults
function definedOptions(): AsciiObjectOptions {
...
onMounted(() => {
  if (canvasEl.value) {
    instance = createAsciiObject({ canvas: canvasEl.value }, definedOptions());
  }
});
...
defineExpose({
  resetView: () => instance?.resetView(),
});

watch(
  () => ({ ...props }),
  () => instance?.setOptions(definedOptions()),
  { deep: true },
);
</script>
